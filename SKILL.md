...
- Keep release documents concise: only the resources to publish (PR links, configuration key/value/description, and other applicable resources) and rollout order. Sync PR links and status into `documents.release.code`; do not leave them only in events or conversations. Omit unchanged resource types and keep historical evidence in the ledger.
