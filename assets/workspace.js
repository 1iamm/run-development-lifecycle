(() => {
  ...
  const execution=t=>t.status==='done'?'':`<span class="execution ${t.status}"><span><strong>${t.status==='blocked'?'等待原因':t.statusSource==='recorded'?'Agent 当前工作':'状态说明'}</strong> ${esc(t.statusReason)}</span>${t.status==='blocked'?`<span class="required-action"><strong>需要你</strong> ${esc(t.requiredAction)}</span>`:''}</span>`;
  const concerns=t=>t.concerns?.length?`<details class="history concerns"><summary>问题与检查记录 · ${t.concerns.length} 项</summary><p class="muted">这些记录保留问题证据；是否等待你处理，以当前执行状态为准。</p>${t.concerns.map(c=>`<p><span class="muted">${esc([c.phase,c.kind,c.id].filter(Boolean).join(' · '))}</span><br>${esc(c.summary)}</p>`).join('')}</details>`:'';
  ...
  function render(){
    ...
    $('#summary').textContent=view==='board'?`${new Set(active.map(t=>t.project)).size} 个项目 · ${active.filter(t=>t.status==='active').length} 项进行中 · ${active.filter(t=>t.status==='blocked').length} 项等你处理 · ${active.filter(t=>t.status==='done').length} 项已完成待归档`:`${archived.length} 项任务 · 由你手动归档`;
    ...
    $('#board').innerHTML=kinds.map((kind,index)=>{
      ...
      return `<section class="column"><div class="column-heading"><h2><span class="dot" aria-hidden="true"></span>${labels[index]}</h2><span class="column-count">${group.length}</span></div><div class="cards">${group.map(t=>`<article class="task-card-shell"><button type="button" class="task-card" data-task="${t.key}" aria-pressed="${selected===t.key}"><span class="project-name">${esc(t.project)}</span><span class="task-name">${esc(t.title)}</span>${badge(t)}${execution(t)}<div>${progress(t)}</div>${t.statusSource!=='recorded'||t.archive?.error?`<span class="card-next">${esc(t.status==='done'?'已完成，等待你手动归档':t.nextAction||'待更新下一步')}${t.archive?.error?`<br>归档未完成：${esc(t.archive.error)}`:''}</span>`:''}<span class="card-meta"><span>${esc(t.taskId)}</span><span>${esc(t.activePhase)}</span></span></button><div class="card-actions">${archiveButton(t,true)}</div></article>`).join('')||'<p class="empty">暂无任务</p>'}</div></section>`;
    }).join('');
    $('#task-count').textContent=active.length+' 项';
    $('#task-table').innerHTML=active.map(t=>`<tr class="${selected===t.key?'selected':''}"><td><button class="text-button" type="button" data-task="${t.key}">${esc(t.title)}</button><div class="task-id">${esc(t.taskId)}</div></td><td>${esc(t.project)}</td><td>${labels[kinds.indexOf(t.stage)]} · ${esc(t.activePhase)}</td><td class="table-status">${badge(t)}${execution(t)}</td><td class="table-progress">${progress(t)}</td><td class="table-next">${esc(t.status==='done'?'等待你手动归档':t.nextAction)}</td><td>${esc(t.targetDate||'待设置')}</td><td><div class="row-actions">${t.thread_id?`<a href="codex://threads/${encodeURIComponent(t.thread_id)}">对话 ↗</a>`:'<span class="muted">未绑定对话</span>'}${archiveButton(t,true)}</div></td></tr>`).join('')||'<tr><td colspan="8" class="empty">新建或导入 lifecycle 任务后，将自动出现在这里。</td></tr>';
  ...
  async function loadDetail(key){
    ...
    try{
      ...
      container.innerHTML=`<div class="detail-head"><div class="eyebrow"><span>${esc(t.project)} / ${esc(t.taskId)}</span><button type="button" class="close" data-close aria-label="收起任务详情">×</button></div><h2>${esc(t.title)}</h2>${badge(t)} <span class="badge">${labels[kinds.indexOf(t.stage)]}</span><div class="detail-actions">${conversation(t)}${archived?'':archiveButton(t)}<span class="small muted">${archived?`本地副本${expired?'已清理':'保留至 '+date(t.archive.expires_at)}`:'任意阶段可手动归档 · 保留当前进度 · 本地副本保留 30 天'}</span></div></div><div class="detail-body"><section><h3>${archived?'归档记录':'阶段与下一步'}</h3><div class="phases">${phaseRows}</div>${execution(t)}${t.executionUpdatedAt?`<p class="small muted execution-time">执行状态更新于 ${esc(new Date(t.executionUpdatedAt).toLocaleString('zh-CN'))}</p>`:''}<div class="next"><span class="muted">${archived?'已归档 · 对话保留':'下一步'}</span><p>${esc(archived?'归档于 '+date(t.archive.archived_at):t.status==='done'?'已完成，等待你手动归档':t.nextAction)}</p></div>${t.archive?.error?`<p class="small">归档错误：${esc(t.archive.error)}</p>`:''}${concerns(t)}<details class="history"><summary>最近记录 · 共 ${d.events.length} 条</summary>${d.events.slice(-12).reverse().map(e=>`<p><span class="muted">${date(e.at)}</span> ${esc(e.summary)}</p>`).join('')}</details></section><section><div class="section-heading"><h3>交付物</h3><span class="small muted">独立页面</span></div>${artifacts}</section></div>`;
